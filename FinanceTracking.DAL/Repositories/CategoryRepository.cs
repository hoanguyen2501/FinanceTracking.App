...
namespace FinanceTracking.DAL.Repositories
{
    public sealed class CategoryRepository : AbstractEntityRepository<CategoryEntity>, ICategoryRepository
    {
        ...
        private async Task<CategoryEntity?> GetEntityByIdAsync(string id, bool isNoTracking = true, CancellationToken ct = default) =>
            await GetQueryable(isNoTracking).FirstOrDefaultAsync(e => e.Id == id, ct);

        private async Task<IEnumerable<CategoryEntity>> GetEntitiesByIdsAsync(IEnumerable<string> ids, bool isNoTracking = true, CancellationToken ct = default) =>
        ...
        protected override IQueryable<CategoryEntity> GetQueryable(bool isNoTracking = false) =>
            Context.Categories.AsNoTracking(isNoTracking)
